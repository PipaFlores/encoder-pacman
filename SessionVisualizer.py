...
class SessionVisualizer:
    def __init__(self, data, game_id=None, playback_speed=1.0, columns=None, verbose = False, Astar=True, pellets=True, show_grid=False):
        """
    ...
    def animate_session(self):
        ...
        for x, y in self.wall_positions:
            ax_game.add_patch(plt.Rectangle((x, y), 1, -1, 
                                          color='blue', alpha=0.5)) # FIXME: Check this position transformation
        
        if self.pellets:
            self.pellet_objects = []
            for x, y in self.pellet_positions:
                pellet = ax_game.plot([x+0.5], [y-0.5], 'o', color='white', markersize=2)[0] # FIXME check this position transformation
                self.pellet_objects.append((pellet, (x, y)))

        pacman_dot, = ax_game.plot([], [], 'o', color='yellow', label='Pac-Man')
        ghost_dots = [ax_game.plot([], [], 'o', label=f'Ghost {i+1}')[0] for i in range(4)]

        path_lines = [ax_game.plot([], [], 'o', color='gray', alpha=0.5, markersize=2)[0] for _ in range(len(ghost_dots))]
        ...
        # TODO: Include other indicators such as score, level and lives.
        ...
        def init():
            ...
            return [pacman_dot, *ghost_dots, game_id_text, time_elapsed_text, *path_lines]
        
        def update(frame):
            if not self.is_playing:
                return [pacman_dot, *ghost_dots, game_id_text, time_elapsed_text, *path_lines,*[p[0] for p in self.pellet_objects]]
            ...
            try:
                ...
                if self.Astar:
                    ...
                    results = Astar.calculate_ghost_paths_and_distances((pacman_x, pacman_y), ghost_positions, self.wall_positions)
                    ...
                    for path_line, path in zip(path_lines, paths):
                        if path:
                            ...
                            path_x = [x + 0.5 for x in path_x] # Transform to unity reference.
                            path_y = [y - 0.5 for y in path_y]
                ...
                if self.pellets:
                    ...
                    for pellet, (x, y) in self.pellet_objects:
                        ...
                        if distance < 0.55 and (x, y) not in self.eaten_pellets:
                ...
                artists = [pacman_dot, *ghost_dots, game_id_text, time_elapsed_text, *[p[0] for p in self.pellet_objects]]
                for path_line in path_lines:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-astar', action='store_false', dest='astar', help="Disable A* path calculation")
    ...
    visualizer = SessionVisualizer(gamestate_df, game_id=args.game_id, playback_speed=args.playback_speed, verbose=args.verbose, pellets=args.pellets, Astar=args.astar, show_grid=args.grid)
