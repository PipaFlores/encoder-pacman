...
def render_video_for_level(level_id):
    ...
    if not os.path.exists(video_path) or os.path.getsize(video_path) < 100 :
        try:
            ...
            # Check file size after rendering
            if not os.path.exists(video_path) or os.path.getsize(video_path) < 80 * 1024:  # 100 KB
                print(f"Error: Rendered video for level_id {level_id} is too small (<100KB), likely an error.")
                if os.path.exists(video_path):
                    try:
                        os.remove(video_path)
                        print(f"Incomplete or corrupt video {video_path} deleted.")
                    except Exception as del_e:
                        print(f"Failed to delete small video {video_path}: {del_e}")
