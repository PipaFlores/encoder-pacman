...
from bokeh.models import ColorBar, LinearColorMapper, CategoricalColorMapper, ColumnDataSource

from bokeh.palettes import Viridis256
print("loading cluster_visualizer")
...
class ClusterVisualizer(BaseVisualizer):
    ...
    def plot_affinity_matrix_bokeh(self):
        ...
        row_indices, col_indices = np.meshgrid(np.arange(shape[0]), np.arange(shape[1]), indexing='ij')
        ...
        p = figure(title="Affinity Matrix",
                   x_range= (0, shape[0]),
                   y_range= (shape[1], 0),
                   tooltips=[("x_index", "@x_index"), ("y_index", "@y_index"), ("value", "@image")],
    ...
    def plot_trajectories_bokeh(self,
                                 traj_centroids: np.ndarray):
